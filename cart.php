...
<?php include("configs/config.php"); ?> 
 <!-- Retrieve and display the stored guest/user ID -->
<script>
document.addEventListener("DOMContentLoaded", function() {
    // Use a consistent key for guest cart ID (here "unique_id")
    const uniqueId = localStorage.getItem('unique_id');
    const userId = localStorage.getItem('user_id');
    const idToDisplay = uniqueId || userId;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Use the consistent key 'unique_id' for guest users
    let guestId = localStorage.getItem('unique_id');
    let backendDomain = "https://haneri.dotcombusiness.in";

    // If user is not logged in and guestId exists, set the cart_id cookie
    if (!token && guestId) {
            ...
            success: function (data) {
                if (data.data && data.data.length > 0) {
                    ...
                    data.data.forEach((item, index) => {
                        ...
                        // Use an indexed image or fallback placeholder
